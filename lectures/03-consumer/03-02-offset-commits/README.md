...
Если нужны вообще нулевые дубли — одной commit-стратегией не обойтись. Тут уже подключают exactly-once через транзакции ([Транзакции и EOS](../../04-reliability/04-01-transactions-and-eos/README.md)) или идемпотентный обработчик ([Гарантии обработки](../../03-consumer/03-03-processing-guarantees/README.md) — следующая лекция). Голый offset commit на стороне консьюмера никогда не даст exactly-once, как ни настраивай.
...
Эта лекция закрыла механику commit'ов. Следующая ([Гарантии обработки](../../03-consumer/03-03-processing-guarantees/README.md)) — про то, как одной commit-стратегией exactly-once не сделаешь, и зачем в обработчике нужна идемпотентность плюс dedup-таблица. Там Postgres, `INSERT ... ON CONFLICT DO NOTHING` и kill -9 в середине обработки уже не дают дублей в БД — потому что обработчик защищён, а не консьюмер.
