...
Если coordinator не получил heartbeat'а от члена дольше, чем `SessionTimeout` (`session.timeout.ms`), он объявляет члена мёртвым и начинает ребаланс без него. Дефолт session.timeout = 45 секунд (Kafka 3.0+), heartbeat.interval = 3 секунды. То есть надо пропустить ~15 heartbeat'ов подряд, чтобы тебя выкинули. Это запас на сетевые задержки и паузы GC — короткий блип сети не должен приводить к ребалансу.

Третий тайминг — `MaxPollInterval` (`max.poll.interval.ms`). Это другой механизм. Дефолт — 5 минут. Он смотрит на интервал между **двумя последовательными вызовами PollFetches**. Если приложение зависло внутри обработки одной партии и не вернулось за PollFetches за 5 минут — coordinator считает его мёртвым, даже если heartbeat'ы продолжали лететь. Идея в том, что heartbeat'ы идут из фоновой горутины и про реальную обработку ничего не знают; max.poll.interval — единственная защита от случая «приложение жуёт одно сообщение полчаса».

Соотношение трёх таймеров на словах:

- heartbeat.interval — как часто стучусь, что я живой;
- session.timeout — насколько долго брокер ждёт стук;
- max.poll.interval — насколько долго брокер ждёт, что я попрошу следующую партию.

Если у тебя обработка одной записи реально может занять 10 минут — поднимай max.poll.interval. Если боишься, что зомбированный процесс будет долго висеть в группе и не отдавать партиции — снижай session.timeout. Если боишься false positives на сетевых блипах — поднимай session.timeout (но и max.poll.interval потяни, чтобы не схлопнулись).
...
3. `OnPartitionsLost` — отличается от `Revoked` ровно одним: его дёргают, когда coordinator уже посчитал тебя мёртвым (пропустил session timeout, например). Партиции не «отзываются по плану», а потеряны. Коммитить offset'ы тут уже нельзя — coordinator откажет.
...
- Тайминги ребаланса — три независимые ручки: `heartbeat.interval` («как часто стучусь»), `session.timeout` («сколько брокер ждёт стук»), `max.poll.interval` («сколько брокер ждёт следующий PollFetches»). Heartbeat и session — про «жив ли вообще». Max.poll.interval — про «не завис ли в обработке».
...
В следующей лекции ([Коммиты offset'ов](../../03-consumer/03-02-offset-commits/README.md)) спустимся на уровень offset-коммитов: разберём auto-commit и его дубли при рестарте, manual sync/async, MarkCommitRecords + CommitMarkedOffsets и пару других ручек. Тут мы offset'ы трогали по касательной — пора разобраться предметно.
