// loud-member — «громкий» член консьюмер-группы, который печатает каждое
// событие ребаланса и текущий список ассайнов.
//
// Идея — запустить три копии в трёх терминалах и посмотреть, как Kafka
// перераспределяет 6 партиций по мере появления и ухода участников группы.
// Каждая копия отличается переменной окружения MEMBER_ID — она попадает
// в ClientID и в каждую строку лога, чтобы было видно, кто именно
// что-то делает.
//
// Стратегия балансировки переключается флагом -strategy:
//   - sticky               (eager) — стандартная sticky-стратегия;
//                          при ребалансе все партиции сначала отзываются,
//                          потом раздаются заново.
//   - cooperative-sticky    — incremental cooperative; отзывается только то,
//                          что реально переезжает к другому члену.
//
// Управление ребалансом:
//   - запустил один процесс  → ему достались все 6 партиций;
//   - стартанул второй       → 6 разделились пополам, видны Revoked + Assigned;
//   - стартанул третий       → 6 разбились на 2/2/2;
//   - убил один (Ctrl+C)      → его партиции уехали к оставшимся.
//
// Чтобы убить процесс не из терминала, можно через PID:
//
...
// pickBalancer возвращает один балансировщик по короткому имени. Cooperative
// и eager на проводе — это разные ребаланс-протоколы; нельзя смешивать в
// одной группе, поэтому держим строго один на запуск.
...
// printEvent красиво печатает map партиций по топикам.
