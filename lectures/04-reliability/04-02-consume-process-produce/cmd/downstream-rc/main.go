// downstream-rc — read_committed-консьюмер на cpp-orders-enriched.
//
// Считает обогащённые записи и проверяет, что каждый ключ встретился
// ровно один раз. Цель — показать наблюдаемый эффект EOS-пайплайна: даже
// если cpp-pipeline аборнул несколько транзакций по дороге (через crash),
// downstream видит ровно столько уникальных ключей, сколько было на
// входе. Без дублей и без пропусков.
//
// Сравнение настройки: -isolation=committed (по умолчанию) фильтрует
// записи аборнутых транзакций; -isolation=uncommitted показывает их —
// видно дельту, на которой работает атомарность транзакций.
package main
