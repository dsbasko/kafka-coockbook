...
Действие. Воркеров мало — добавляешь копии (или партиций больше — но это уже про планирование, не runbook). Один поток обработки тяжёлый — смотришь, можно ли распараллелить per-key через worker pool из лекции [Конкурентность и lag](../../03-consumer/03-05-concurrency-and-lag/README.md). Внешняя система (БД, HTTP) тормозит — проверяешь её, поднимаешь backpressure через `cl.PauseFetchPartitions` (см. [Доставка во внешние системы](../../04-reliability/04-05-external-delivery/README.md)), а не молча копишь in-flight.
...
Действие. (1) — поднимаешь `max.poll.interval.ms` или ускоряешь обработку (то же `cl.PauseFetchPartitions`, разделение тяжёлой работы на воркеров). (2) — поднимаешь `session.timeout.ms`, разбираешься с GC. (3) — переходишь на `cooperative-sticky` (`kgo.Balancers(kgo.CooperativeStickyBalancer())`), чтобы при ребалансе двигались только перераспределяемые партиции, а не все. Это всё было в [Группы и ребалансы](../../03-consumer/03-01-groups-and-rebalance/README.md), тут только напоминалка.
...
- `InvalidProducerEpoch` — кто-то ещё стартанул с тем же `transactional.id`. Это zombie fencing, см. [Транзакции и EOS](../../04-reliability/04-01-transactions-and-eos/README.md).
...
- Топик с `retention.ms=-1` (compact или просто бесконечность). Размер сегментов — нормальная картина для CDC-state, но за ней тоже надо следить (см. профили в [Sizing и tuning](../../08-operations/08-03-sizing-and-tuning/README.md)).
- Retention настроен, но не срабатывает — `segment.ms` слишком большой, активный сегмент не закрывается, retention его не трогает (см. [Retention и compaction](../../08-operations/08-02-retention-and-compaction/README.md)). Вижу 70% диска под одним сегментом — обычно это.
