...
Лекция [Retention и compaction](../../08-operations/08-02-retention-and-compaction/README.md) была про то, как Kafka режет лог на сегменты и что с ними потом делает retention/compaction. Эта — на ступень выше. Тут уже не «как работает retention», а «какой retention я ставлю на этот топик и почему именно такой». Пара ручек на топике решает, во что превратится диск и как кластер ведёт себя в инциденте. Хочется понимать, что выбирает оператор.
...
Две политики из [Retention и compaction](../../08-operations/08-02-retention-and-compaction/README.md) — `delete` и `compact`. Бывает ещё `compact,delete` (и compact, и retention доедает старое). Выбор — это в первую очередь ответ на вопрос «что в топике лежит». Поток событий — `delete`. Снимок состояния по ключу — `compact`. Снимок с TTL «удалять профили старше года, даже если tombstone не пришёл» — `compact,delete`.
...
Сегмент — это файл, в который пишет активный кусок партиции. Закрывается по достижении `segment.bytes` (1 ГБ дефолт) или по возрасту `segment.ms` (неделя дефолт). Retention и compaction трогают только закрытые сегменты — это крутили в [Retention и compaction](../../08-operations/08-02-retention-and-compaction/README.md).
...
Это не «правильные» цифры — это рабочие отправные точки. Дальше крутишь под свой профиль трафика и смотришь на диск через `DescribeAllLogDirs` и на лаг через `kadm.Lag`.
