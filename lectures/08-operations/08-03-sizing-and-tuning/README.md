...
Две политики из 08-02 — `delete` и `compact`. Бывает ещё `compact,delete` (и compact, и retention доедает старое). Выбор — это в первую очередь ответ на вопрос «что в топике лежит». Поток событий — `delete`. Снимок состояния по ключу — `compact`. Снимок с TTL «удалять профили старше года, даже если tombstone не пришёл» — `compact,delete`.
...
Сегмент — это файл, в который пишет активный кусок партиции. Закрывается по достижении `segment.bytes` (1 ГБ дефолт) или по возрасту `segment.ms` (неделя дефолт). Retention и compaction трогают только закрытые сегменты — это крутили в 08-02.
...
`CreateTime` нужен, когда event-time важен для downstream — например, для windowing в stream-обработке (см. лекцию 07-01). `LogAppendTime` — когда вы не доверяете часам клиентов и retention важнее event-time. На метриках с агрессивным TTL `LogAppendTime` стабильнее: retention режет по «времени брокера», а не по тому, что нарисует продьюсер с поехавшими часами.
