...
Сравни с [Postgres → ClickHouse с анонимизацией](../../09-use-cases/03-pg-to-clickhouse/README.md): то же количество звеньев в Connect'е, только вместо ClickHouse Sink'а — ES Sink, и плюс цепочка SMT, потому что ES любит плоский документ, а Debezium шлёт обёрнутый. Postgres-схема, publication, replication slot, RF — без изменений.
...
Schema Registry — здесь Debezium и Sink говорят через JsonConverter. Это удобно для дебага (открыл консоль и читаешь). Под нагрузку JSON неэффективен, и обычно цепочку переводят на Avro через SR — оба коннектора это поддерживают, надо только заменить пары `*.converter` и поднять SR (он уже есть в корневом стенде, лекция [Schema Registry](../../05-contracts/05-03-schema-registry/README.md) про него отдельно).
