...
2. **Tooling.** `buf` (которым мы займёмся в [Protobuf в Go](../../05-contracts/05-02-protobuf-in-go/README.md)) — это современный, удобный инструмент: lint, breaking-change detection, форматирование, удобный workflow. У Avro эквивалентов меньше и они тяжелее.
...
Числа `1, 2, 3, 4, 5, 6` — это field-номера из `proto/order.proto`. Никакой больше связи с `.proto`-файлом тут нет — мы вручную делаем то, что обычно делает кодген. В [Protobuf в Go](../../05-contracts/05-02-protobuf-in-go/README.md) заменим всё это на сгенерированный `proto.Marshal`.
...
Дальше в модуле 05 разберём Protobuf в Go ([Protobuf в Go](../../05-contracts/05-02-protobuf-in-go/README.md)), Schema Registry с magic byte и schema_id ([Schema Registry](../../05-contracts/05-03-schema-registry/README.md)) и эволюцию схем без поломки совместимости ([Эволюция схем](../../05-contracts/05-04-schema-evolution/README.md)). К концу модуля у вас будет рабочий setup, в котором изменение `.proto`-файла проверяется CI и регистрируется в Registry автоматически.
...
`proto-gen` — намеренно no-op в этой лекции. В [Protobuf в Go](../../05-contracts/05-02-protobuf-in-go/README.md) заменим вручное wire-кодирование на сгенерированный protoc-gen-go код, и `proto-gen` станет настоящей целью с buf.
