...
Печать сама по себе — обычные generated getter'ы, как в [Protobuf в Go](../../05-contracts/05-02-protobuf-in-go/README.md):
...
- `gen/` — в репо. Те же аргументы, что и в [Protobuf в Go](../../05-contracts/05-02-protobuf-in-go/README.md): воспроизводимость без `buf` на чистом clone'е, ревьюер видит, как сгенерёный код реагирует на правку схемы.

## Что дальше

В [Эволюция схем](../../05-contracts/05-04-schema-evolution/README.md) — про эволюцию схем. Что значит BACKWARD/FORWARD/FULL compatibility, какие правки в Protobuf SR пропустит, а какие — нет. И как `buf breaking` ловит ломающие изменения ещё до того, как они доедут до SR.
