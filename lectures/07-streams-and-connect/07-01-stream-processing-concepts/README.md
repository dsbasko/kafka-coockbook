...
В нашем коде watermark'а нет — мы просто бесконечно копим окна в памяти и печатаем. Это упрощение для лекции. В 07-02 watermark'а тоже не будет — для word-count он не нужен. Реальные fully-fledged системы (Flink, Beam, Kafka Streams 3.x+) считают watermark'и автоматически.
...
Под капотом KTable обычно — compacted-топик плюс локальный state store (RocksDB или Pebble), куда записи проецируются. При рестарте store восстанавливается из compacted-топика. В 07-02 мы повторим эту схему руками: Pebble + changelog topic для word-count.
...
В 07-02 мы добавим Pebble и changelog topic — получится рабочая модель stateful processing'а с восстановлением state'а после рестарта. Без этого слоя любая аналитика в стриме обнуляется при первом же сбое.
...
В 07-02 возьмём это и положим на стрим, который реально хранит state — word-count на franz-go + Pebble + changelog topic. После рестарта state восстанавливается из changelog, и накопленные подсчёты не теряются. Это уже близко к настоящему stream processing'у.
