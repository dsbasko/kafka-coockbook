...
Это и есть stream processing. Лекция вводная — с кодом разберёмся в [Stream processing на Go (franz-go + Pebble)](../../../07-02-stream-processing-in-go/i18n/ru/README.md). Тут — про идеи. Что такое event-time, чем он отличается от processing-time, что такое windowing, почему он почти всегда event-time-based, что значит «late events» и зачем нужен watermark. Плюс короткие сноски про KStream/KTable, repartitioning и stateful operations — это словарь, без которого [Stream processing на Go (franz-go + Pebble)](../../../07-02-stream-processing-in-go/i18n/ru/README.md) будет читаться как иероглифы.
...
В нашем коде watermark'а нет — мы просто бесконечно копим окна в памяти и печатаем. Это упрощение для лекции. В [Stream processing на Go (franz-go + Pebble)](../../../07-02-stream-processing-in-go/i18n/ru/README.md) watermark'а тоже не будет — для word-count он не нужен. Реальные fully-fledged системы (Flink, Beam, Kafka Streams 3.x+) считают watermark'и автоматически.
...
Под капотом KTable обычно — compacted-топик плюс локальный state store (RocksDB или Pebble), куда записи проецируются. При рестарте store восстанавливается из compacted-топика. В [Stream processing на Go (franz-go + Pebble)](../../../07-02-stream-processing-in-go/i18n/ru/README.md) мы повторим эту схему руками: Pebble + changelog topic для word-count.
...
В [Stream processing на Go (franz-go + Pebble)](../../../07-02-stream-processing-in-go/i18n/ru/README.md) мы добавим Pebble и changelog topic — получится рабочая модель stateful processing'а с восстановлением state'а после рестарта. Без этого слоя любая аналитика в стриме обнуляется при первом же сбое.
...
В [Stream processing на Go (franz-go + Pebble)](../../../07-02-stream-processing-in-go/i18n/ru/README.md) возьмём это и положим на стрим, который реально хранит state — word-count на franz-go + Pebble + changelog topic. После рестарта state восстанавливается из changelog, и накопленные подсчёты не теряются. Это уже близко к настоящему stream processing'у.
