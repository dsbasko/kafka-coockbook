...
В [Acks и durability](../../02-producer/02-02-acks-and-durability/README.md) мы измеряли latency на честных round-trip'ах через ProduceSync — каждый record отдельно, по одному в полёте. Это была демка про acks, не про скорость. Тут берём другую сторону медали — пропускную способность. Что вообще даёт продьюсеру скорость, как пользоваться батчингом и где он начинает мешать.
...
1. Накопилось `ProducerBatchMaxBytes` (по умолчанию 1 МБ) — отправляем сразу, нет смысла копить.
2. Сработал `ProducerLinger` — таймер, по истечении которого отправляется всё, что есть. По дефолту — 0, то есть «отправляй как только сможешь».

Дефолт linger=0 не значит «без батчинга». Когда несколько горутин одновременно пишут в один и тот же продьюсер, между моментом «положил в очередь» и моментом «продьюсер собрался отправить запрос» проходит крошечное время — и в это окно успевает упасть ещё несколько сообщений. Так получается естественный батч, без задержки на стороне приложения. Но если сообщения идут редко — каждое полетит само по себе.

`ProducerLinger > 0` принудительно ждёт, пока истечёт этот таймер, накапливая больше. На очень частом потоке прибавки почти нет (батч и так толстый), на медленном — таймер съедает latency. Поэтому linger — это компромисс между «отправить мгновенно» и «отправить эффективно».
...
- Толстые батчи получаются легче, когда сообщений в одну партицию много. На разреженном паттерне «по ключу один-два-record'а в секунду» батч никогда не наберётся, и linger=0 будет отправлять каждое почти сразу.
- При sticky-partitioner (дефолт franz-go при пустом ключе) продьюсер некоторое время «прилипает» к одной партиции — кладёт туда подряд несколько record'ов. Это специально, чтобы батч хоть иногда набирался даже без ключа.
...
Запись принципиально асинхронная — `cl.Produce` плюс callback. Если бы мы делали ProduceSync (как в [Acks и durability](../../02-producer/02-02-acks-and-durability/README.md)), батч-эффекты бы исчезли: пока один record не добьёт round-trip, следующий не уезжает.
...
Это ОК для нашего сравнения — мы сравниваем сценарии между собой при одинаковой нагрузке. Если хочется честную per-record latency на низкой нагрузке — это уже другой эксперимент (и он был в [Acks и durability](../../02-producer/02-02-acks-and-durability/README.md) с ProduceSync).
...
- linger=0 — это не «без батчинга». Это «отправляй когда сможешь». Естественные батчи всё равно набираются, просто меньше.
- linger > 0 нужен для **разреженного потока**, чтобы дать накопиться. На плотном потоке от него только хвостовая latency растёт — дешевле оставить 0.
...
В [Ошибки, retries и headers](../../02-producer/02-05-errors-retries-headers/README.md) разберёмся с классами ошибок продьюсера, retry/timeout-настройками и headers — это последний кусок, после которого продьюсер у нас будет полным.
