...
С идемпотентностью самой по себе тут ничего не поделать. Защита от зомби — это **transactional.id** + producer-epoch fencing, тема следующего модуля ([Транзакции и EOS](../../04-reliability/04-01-transactions-and-eos/README.md)). Идемпотентность спасает от дублей **внутри** одной сессии продьюсера, не между сессиями.
...
В дефолтной ветке (`-idempotent=true`) дополнительно ничего не добавляется — franz-go сам включит идемпотентность с `acks=all`. В альтернативной ветке нужно явно выключить идемпотентность и явно поставить `acks=all` (иначе клиент будет ругаться на несовместимые дефолты). `acks=all` я держу одинаковым в обоих режимах — чтобы исключить, что разница в логе вылезла из-за разной durability, а не из-за идемпотентности.
...
В [Батчинг и пропускная способность](../../02-producer/02-04-batching-and-throughput/README.md) разберёмся с батчингом и компрессией — оттуда же поймём, почему `max.in.flight ≤ 5` практически не влияет на throughput.
