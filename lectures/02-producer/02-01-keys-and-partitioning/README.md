...
Что важно. Во-первых, у каждого ключа ровно одна партиция — `one-key-one-partition ✓` стоит у всех. Это и есть гарантия дефолтного partitioner'а. Во-вторых, между ключами раскладка детерминированная: `user-0` всегда в партиции 2 (на 3-партиционном топике с murmur2), `user-1` всегда в 0, и так далее. На 4 партициях раскладка будет другая — и та же. На 6 партициях — снова другая. Меняется N — меняется всё.

В-третьих, по партициям получилось 400/300/300 — не идеально ровно. Это нормально на 10 уникальных ключах: 10 — слишком мало для равномерного хеша на 3 партициях. На 1000 уникальных ключей было бы заметно ровнее.
...
Толерантность для round-robin'а — 5% от суммы. Тут разъехалось на 1 запись из 699 — отлично.
...
Цифры под `--premium-pct=30` пляшут вокруг 300/350/350. На других значениях `-premium-pct` будет другой расклад.
...
В следующей лекции ([Acks и durability](../../02-producer/02-02-acks-and-durability/README.md)) — про `acks` и durability. Тоже одна крошечная опция, которая определяет, потеряешь ты данные при падении брокера или нет.
