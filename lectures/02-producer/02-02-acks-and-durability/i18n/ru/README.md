...
В прошлой лекции ([Ключи и партиционирование](../../../02-01-keys-and-partitioning/i18n/ru/README.md)) ключ определял, **куда** ляжет запись. Сейчас разберём другую крошечную опцию — `acks`. Она определяет совсем другой вопрос: **когда** продьюсер считает, что запись «уже всё, записана». И от ответа на этот вопрос зависит, потеряешь ты данные при падении брокера или нет.
...
Подробнее про идемпотентный продьюсер — в следующей лекции ([Идемпотентный продьюсер](../../../02-03-idempotent-producer/i18n/ru/README.md)). Тут просто запомни: дефолт franz-go = идемпотентный продьюсер с acks=all. Чтобы ослабить acks, нужно дополнительно отключить идемпотентность.
...
Throughput идёт обратно: 23k/1.4k/0.8k msg/s. Цифры выглядят пугающе низкими, но мы синхронные — без батчинга и параллелизма. С нормальным async-продьюсером и линджером — другая история, в [Батчинг и пропускная способность](../../../02-04-batching-and-throughput/i18n/ru/README.md) будем мерить именно throughput.
...
- Идемпотентность в franz-go по дефолту включена и требует `acks=all`. Чтобы понизить — `kgo.DisableIdempotentWrite()`. Сама идемпотентность — тема следующей лекции ([Идемпотентный продьюсер](../../../02-03-idempotent-producer/i18n/ru/README.md)).

В [Идемпотентный продьюсер](../../../02-03-idempotent-producer/i18n/ru/README.md) разберём, что делает идемпотентный продьюсер на самом деле — почему он спасает от дублей при ретраях и почему **не** спасает от zombie-сценария между сессиями.
