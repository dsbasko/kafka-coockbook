// consumer — первый консьюмер курса. Читает топик в группе, печатает каждую
// запись в виде partition/offset/key/value и корректно завершается по SIGINT.
//
// По умолчанию подписан на тот же топик, в который пишет 01-05
// (lecture-01-05-first-producer), чтобы две лекции стыковались: запустил
// продьюсер, потом запустил консьюмер — увидел те же 10 строк.
//
// Группа задаётся флагом -group (по умолчанию lecture-01-06-group). При
// первом старте группы offset сбрасывается на earliest (флаг -from-start,
// включён по умолчанию) — иначе на свежей группе мы бы не увидели те
// сообщения, которые продьюсер записал ДО запуска консьюмера. На втором
// запуске committed offset уже есть, поэтому -from-start ничего не меняет
// и консьюмер ждёт новых сообщений.
//
// MEMBER_ID — переменная окружения для метки в выводе (нужна, чтобы
// различать терминалы при запуске двух копий в одной группе).
package main
