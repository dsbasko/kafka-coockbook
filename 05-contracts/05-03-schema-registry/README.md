...
Subject — это «логическое имя контракта». Конвенция по умолчанию (TopicNameStrategy) — `<topic>-value` для значений и `<topic>-key` для ключей. Внутри одного subject'а живёт цепочка версий — добавил поле в схему, зарегистрировал новую версию, получил новый id, старые id никуда не делись. Об эволюции — 05-04.
...
Сначала — нулевой magic byte. Дальше четыре байта `schema_id` в big-endian. Дальше — message-index: zigzag-кодированный varint для protobuf, в котором лежит путь к нужному message внутри `.proto`-файла (если в файле несколько message'ей — индекс говорит, какой из них). Для одного top-level message это одинокий байт `0x00`. Только потом — сам сериализованный payload.
...
Печать сама по себе — обычные generated getter'ы, как в 05-02:
...
- `gen/` — в репо. Те же аргументы, что и в 05-02: воспроизводимость без `buf` на чистом clone'е, ревьюер видит, как сгенерёный код реагирует на правку схемы.

## Что дальше

В 05-04 — про эволюцию схем. Что значит BACKWARD/FORWARD/FULL compatibility, какие правки в Protobuf SR пропустит, а какие — нет. И как `buf breaking` ловит ломающие изменения ещё до того, как они доедут до SR.
