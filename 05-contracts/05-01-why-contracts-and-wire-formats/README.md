...
2. **Tooling.** `buf` (которым мы займёмся в 05-02) — это современный, удобный инструмент: lint, breaking-change detection, форматирование, удобный workflow. У Avro эквивалентов меньше и они тяжелее.
...
Числа `1, 2, 3, 4, 5, 6` — это field-номера из `proto/order.proto`. Никакой больше связи с `.proto`-файлом тут нет — мы вручную делаем то, что обычно делает кодген. В 05-02 заменим всё это на сгенерированный `proto.Marshal`.
...
Дальше в модуле 05 разберём Protobuf в Go (05-02), Schema Registry с magic byte и schema_id (05-03) и эволюцию схем без поломки совместимости (05-04). К концу модуля у вас будет рабочий setup, в котором изменение `.proto`-файла проверяется CI и регистрируется в Registry автоматически.
...
`proto-gen` — намеренно no-op в этой лекции. В 05-02 заменим вручное wire-кодирование на сгенерированный protoc-gen-go код, и `proto-gen` станет настоящей целью с buf.
