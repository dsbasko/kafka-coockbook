// manual-sync — консьюмер с выключенным auto-commit и ручным sync-commit'ом
// после каждого батча. Идея простая: пока приложение реально не закончило
// обработку всех записей в батче, offset не двигается. Если упали в
// середине — committed offset остался на начале батча, при рестарте
// прочитаем тот же батч заново, дубли возможны (at-least-once).
//
// Что делает программа:
//   1. Подписывается на топик в группе с DisableAutoCommit().
//   2. Каждый PollFetches возвращает батч; для каждой записи спит -work-delay
//      и пишет строку в processed-sync.log.
//   3. После того как ВЕСЬ батч обработан — вызывает CommitRecords (sync,
//      ждёт ack от брокера).
//   4. Если -crash-after > 0 — после стольких обработанных записей os.Exit(1)
//      БЕЗ commit'а. Это имитирует падение в середине батча.
//
// Тонкость, которая отличает это от auto-commit:
//   - тут commit происходит ПОСЛЕ обработки → если крашнулся в середине,
//     committed offset не сдвинулся → дубли при рестарте.
//   - в auto-commit коммитилось то, что было ПОЛУЧЕНО → если auto-commit
//     успел сработать, на рестарте теряем недообработанные записи.
//
// Цена sync-commit'а — поход в брокер на каждый батч и блокировка цикла.
// Если батчи маленькие, это серьёзный оверхед. Для больших батчей
// (сотни-тысячи записей) — нормальная цена за чёткие гарантии.
package main
...
func run(ctx context.Context, o runOpts) error {
	...
	for {
		...
		// Сначала собираем все записи батча — потом по очереди обрабатываем.
		// Делаем это в массив, чтобы знать размер батча и иметь полный
		// набор для CommitRecords в конце.
		...
		// Точка коммита: вся пачка обработана и сохранена в лог,
		// теперь сдвигаем committed offset.
